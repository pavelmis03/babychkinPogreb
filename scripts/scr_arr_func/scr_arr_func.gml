/// @function scr_arr_findEl(arr, el, dimension);
...
function scr_arr_findEl(arr, el, dimension) {
	if (dimension == 1) {
		for (var i = 0; i < array_length(arr); i++) {
			if (is_array(el)) {	// если мы ищем в массиве массив (если элемент является массивом)
				if (array_equals(el, arr[i])) {
					return i;
				}
			} else {
				if (el == arr[i]) {
...
function scr_arr_readFromStr(str) {
	...
	var flag = false;	// с какой стороны стоял массив от элементов [a, b, []] - true, [[], a, b] - false;
	...
	while (str != "") {
		...
		if ((a != -1) and (b != -1)) {
			...
			str = string_delete(str, clamp(a - 1, 1, string_length(str)), abs(b + 1 - (a - 1)));
			if (a != 1) {	// если массив стоял справа от элементов
				flag = true;
			}
			...
			if (string_count("[", str) == 0) {	// хз, зачем, на всякий 
				...
				if (flag) {	// в зависимости от того, где стояли элементы относительно массива, соединяем по-разному
					arr = array_concat(tArr, arr);
				} else {
