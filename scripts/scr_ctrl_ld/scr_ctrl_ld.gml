...
function scr_ld_createLdBtn() {
	...
	for (var i = 0; (i < ctrl_ldGm_svOnPage) and (t_num + 1 < ctrl_ldGm_sv); i++) {
		...
		with (obj) {
			...
			var str1 = string_copy(str, 0, string_pos(" ", str) - 1);	// разбираю на дату
			str = string_copy(str, string_pos(" ", str) + 1, 10);	// и время
			if (string_pos("/", str1) != 0) {	// если английский формат времени
				str1 = string_split(str1, "/");		// разделяю на части
				var arr = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
				btn_infoStruct[$"dateTime"] = str1[1] + " " + arr[int64(str1[0]) - 1] + " " + str1[2] + " at " + str;
			} else { // если русский формат времени
				str1 = string_split(str1, ".");		// разделяю на части
				var arr = ["Января", "Февраля", "Марта", "Апреля", "Мая", "Июня", "Июля", "Августа", "Сентября", "Октября", "Ноября", "Декабря"];
				btn_infoStruct[$"dateTime"] = str1[0] + " " + arr[int64(str1[1]) - 1] + " " + str1[2] + " в " + str;
			}			
...
function scr_ld_findSvPaths(path) {	
	...
	var folderCnt = 0;	// количество найденных папок сохранений в игре
		...
		while (fn != "") {
			if (string_pos(".", fn) == 0) { // если найденный путь не файл
				...
				if (string_copy(fn, 0, string_pos("_", fn)) != "branch_") {
					...
					folderCnt++;	// считаем папки сохранений
	...
	// если количество папок внутри игры нулевое, удаляем игру
	if (folderCnt == 0) {
		scr_ld_delGm(ctrl_ldGm_gm_currPath);
	}
