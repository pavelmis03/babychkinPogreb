...
function scr_ld_createObjFromFile(filePath) {
	...
	while (!file_text_eof(file)) {
		...
		s = string_copy(s, 0, string_length(s) - 2);	// строка без символов переноса (добавлено позже - предположение)
		...
		if (s == "") {	// если это была пустая строка
			if (array_length(objVal) != []) {	// если мы собрали объект
				...
				for (var i = 0; i < array_length(keys); i++) {
					...
					if (scr_arr_findEl(["que", "priority", "stack", "map", "list"], val[1], 1) == -1) {
						val[0] = scr_var_convVal(val[0], val[1]);
					}	
					// назначаю в любом случае. Если структура - как строку
					variable_instance_set(obj, keys[i], val[0]);
				}
			}
		} else {	// если мы встретили какой-то текст
			if (s == "INST") {	// если это начало описания
