...
function scr_snd_musicInRm() {
	...
	if (ds_map_exists(ctrl_msc_map_, rm_name)) {
		...
		for (var i = 0; i < array_length(ctrl_msc_arr_on); i++) {
			...
			var ind = scr_arr_findEl(t_arr, audio_get_name(t_msc[0]), 1);
			if (ind != -1) {
...
function scr_snd_playSnd() {
	...
	if ((array_length(sound) > 1) and (sound[0] != -1)) {
		...
		for (var i = 0; i < array_length(sound); i++) {
			if (is_numeric(sound[i])) {	// вытаскиваю приоритет звука, если это число
				// приоритет звуков хранится первым значением
				priority = int64(sound[i]);	// костыль, который правит сохранения
			} else { // если это звук, а не приоритет, проигрываем 
				// если имеем второй случай (см. пример)
				if (is_array(sound[i])) {
					// выбираем один из треков
...
/// @param {} snd_for_check - звук проверки (передавать как строку)
/// @param {} snd_for_play - звук запуска/остановки (передавать как строку)
...
function scr_snd_requestPlaySnd(priority, snd1, snd2, playOrStop) {
	snd1 = asset_get_index(snd1); // нужен нам как ресурс для проверки
	// snd2 нужен, как строка для добавления в массив звуков проигрывания
	if (playOrStop) {	// звук может быть активен, но может не воспроизводиться, если он относится к другой комнате
		...
		if (audio_is_playing(snd1)) {	// останавливаем звук 
			audio_stop_sound(asset_get_index(snd2));
		}
	}
}
