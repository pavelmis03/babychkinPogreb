...
function scr_buh_chechPlayerDir() {
	if (!buh_playerIn) {	// иначе игрок сидит в буханке
		...
		// НОВОЕ УСЛОВИЕ (не тестил) если игрок не атакует и т.д., если свободен, короче
...
/// @description расчитывает для каждой передачи (и на холостых) потребление топлива, для каждой передачи свои коэффициенты
	...
	// https:// 1gai.ru/uploads/posts/2018-12/1543933692_2-4.jpg
function scr_buh_calcFuelConsumption() {
	if (scr_arr_findEl([1, 3], buh_status, 1) != -1) {	// если заведена или едет
		...
		if (buh_transmission != 0) {	// потребление на всех передачах, кроме нейтрали
			...
			var t = (buh_fuelConsumption * t1) / fps / (100 / (abs(speed) * 3.6) * 3600/*за сколько сек. мы проедем 100 км*/);
		...
		if ((buh_transmission == 0) or (buh_coupling)) { 
			...
			obj_ctrl_gm_buh.buh_fuel -= buh_fuelConsumption / fps / 3600;	// потребление в час
		}
	...
	if (flag) {	// дать шанс
		...
		if (abs(speed) > 0.3 * abs(arr[0])) {// если скорость меньше, чем 30% от мин скорости для текущей передачи
			if (alarm_get(1) <= 0) {	// если мы еще не завели таймер
				...
				alarm[1] = irandom_range(2, 5) * fps;
				scr_snd_requestPlaySnd(20, "snd_buh_preStall", "snd_buh_preStall", true);	// звук чихающего движка
		...
		scr_snd_requestPlaySnd(20, "snd_buh_stall", "snd_buh_stall", true);	// звук глохнущего движка
		obj_ctrl_gm_hint.ctrl_hint_newHint = "buhankaStall";	
		scr_snd_requestPlaySnd(20, "snd_buh_idle", "snd_buh_idle", false);	// останавливаем звук холостого двигателя
		scr_snd_requestPlaySnd(20, "snd_buh_movement", "snd_buh_movement", false);	// останавливаем звук движения
		// износ не нужен, он в местах вызова прописан
	}
}

/// @function scr_buh_aniEndFunc(spr);
/// @param {ind} spr спрайт, с которым мы работаем
/// @param {string} snd звук к этому спрайту
...
function scr_buh_aniEndFunc(spr, snd) {
	...
	if (image_speed == -1) {	// если выходим из комнаты взаимодействия
		scr_snd_requestPlaySnd(20, snd, snd, true);	// звук 
