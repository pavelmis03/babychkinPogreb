...
if (scr_arr_findEl([spr_buh_state, spr_buh_stateStarted, spr_buh_broke], sprite_index, 1) != -1) {
	// если персонаж в буханке
	if (buh_playerIn) {
		// чтобы выйти из буханки, нужно остановиться и поставить буханку на нейтраль и ручник
		if ((buh_handbrake) and (buh_transmission == 0)) {
			...
			scr_snd_requestPlaySnd(20, "snd_buh_getInCar", "snd_buh_getInCar", true);	// звук 
		} else {
		...
		switch (can_interact) {
			case 1:// когда она сломана, такого варианта просто не будет (см. скрипт scr_buh_checkPlayerDir())
				...
				image_index = 0;
				scr_snd_requestPlaySnd(20, "snd_buh_getInCar", "snd_buh_getInCar", true);	// звук посадки в машину
				obj_ctrl_gm_hint.ctrl_hint_newHint = "buhankaStart";	
			break;
			case 2:	// открыть багажник
				...
				image_index = 0;
				scr_snd_requestPlaySnd(20, "snd_buh_openTrunk", "snd_buh_openTrunk", true);
				// начинаем взаимодействие
				interacting = true;
			break;
			case 3:	// модернизировать (открыть капот)
				if (obj_ctrl_gm_buh.buh_broke == 2) {	// если машина сломана, то нужен спрайт сломанной буханки
				...
				image_index = 0;
				scr_snd_requestPlaySnd(20, "snd_buh_mode", "snd_buh_mode", true);	
				// начинаем взаимодействие
				interacting = true;
			break;
		}
