/// @description тормоз
if (scr_arr_findEl([1, 2, 3], buh_status, 1) != -1) {	// если заведена или едет или заглохла (тормазить можно)
	...
	if (buh_transmission == 5) {	// реверс
		speed += (CONST_BUH_DEBOOST * t1 * t2) / fps;	// все наоборот т.к. скорость отрицательная
		speed = clamp(speed, -15, 0);
	} else {
		speed -= (CONST_BUH_DEBOOST * t1 * t2) / fps;
		speed = clamp(speed, 0, 120);
	}
	if (speed != 0) {
		scr_snd_requestPlaySnd(20, "snd_buh_squeak", "snd_buh_squeak", true);	// воспроизводим звук срипа тормозов
	} 
}