...
if (obj_ctrl_gm_buh.buh_broke != 2) {	// если она не сломана
	...
	if (speed != 0) {
		...
		// var T = l / (speed * 3.6);	
		// var T = l / 5;	
		// direction += 360 / T / fps * sign(buh_rotAngle);	
		direction += 360 / R * speed / fps / t * sign(buh_rotAngle);	
	...
	if (buh_status == 1) {
		scr_snd_requestPlaySnd(20, "snd_buh_idle", "snd_buh_idle", true);	// звук рева
	}

	// если мы едем
	if (buh_status == 3) {
		obj_ctrl_gm_buh.buh_mileage	+= (speed / 1000 / fps);	// пробег / 1000, потому что км/сек считаю. (км = 1/1000м)
		scr_snd_requestPlaySnd(20, "snd_buh_movement", "snd_buh_movement", true);	// звук движения
		...
		if ((buh_coupling) or (buh_transmission == 0)) {	// если нейтраль или сцепление
			...
			speed -= clamp(t + t1, 0, abs(speed) * tc) / fps * signSpd;
		...
		if (buh_transmissionNeed > buh_transmission) { // если передача ниже нужной
			scr_snd_requestPlaySnd(20, "snd_buh_engineRoar", "snd_buh_engineRoar", true);	// звук ревущего движка
		}
	...
	if (buh_status != 2) {
		...
		if (speed != 0) { // если мы заглохли, но еще не остановились
			...
			speed -= CONST_BUH_DEBOOST * 0.7 / fps * signSpd;	// торможение двигателем
		} else {
	...
	obj_ctrl_gm_buh.hp -= (buh_decomposition + buh_decomposition1) / fps;	// износ в секунду
