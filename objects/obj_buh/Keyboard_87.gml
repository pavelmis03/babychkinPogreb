	...
	if (buh_handbrake) {	// если включен ручник
		...
		if ((!buh_coupling) and (buh_transmission != 0)) {	// если не нейтраль и не сцепление
			...
			if (tt != noone) {	// если под нами определена поверхность
				t1 = tt.surf_params[1][0];	// уменьшенение ускорения по поверхности 
			}
			...
			speed += (CONST_BUH_BOOST * t * t1 * t2) / fps;
			scr_snd_requestPlaySnd(20, "snd_buh_gas", "snd_buh_gas", true);	// воспроизводим звук рычания
		} else {
			obj_ctrl_gm_hint.ctrl_hint_newHint = "buhankagasN";
			scr_snd_requestPlaySnd(20, "snd_buh_gasN", "snd_buh_gasN", true);	// воспроизводим звук рычания на нейтрали
		}
	}
}