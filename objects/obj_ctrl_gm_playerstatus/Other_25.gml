...
switch (action) {
	...
	case "changeRm":	// создаем игрока в новой комнате
		...
		if (scr_arr_findEl(player_arr, arr[1], 1) != -1) {	// проверяем, что в этой комнате должен быть игрок
			...
			if (!instance_exists(arr[1])) {				// проверяем, что игрока нет
				...
				// и если это не новая игра, при ней мы берем изначальные координаты объектов
				if (!player_isNewGm) {
					var t = player_map_info[?room];				// информация об игроке
