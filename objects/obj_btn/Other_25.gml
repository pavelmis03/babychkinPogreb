...
switch (type) {
	...
	break;
	// ИГРА
	// инвентарь ящика и буханки - кнопки взять все и положить все
	case "confirmYNCl_invBox": 
		// если в инвентаре или в контрагенте лежит что-то (смотря куда/откуда) перемещаем предметы
		if (((action[0] == "invPutAll") and (obj_ctrl_gm_inv.inv_weight != 0))
			or ((action[0] == "invTakeAll") and (obj_ctrl_gm_rmInv.inv_weight_other != 0))) {
			// то вызываем диалог
			obj_ctrl_dlg.ctrl_dlg_need = true;
			obj_ctrl_dlg.type = type;
			obj_ctrl_dlg.action = action;
		} else {
			// иначе ничего не делаем и выдаем подсказку, что нечего перемещать
			obj_ctrl_dlg.ctrl_dlg_del = true;
			var tarr = ["box_nothing_move1", "box_nothing_move2", "box_nothing_move3"];
	...
	case "transition": 
		...
		if (btn_transitionDelay == 0) {
			...
			alarm[7] = btn_transitionDelay * fps;
		} 
	break;
