/// @description 

//объект игрока
player_obj = scr_player_type();

//если персонаж не существует, рисовать нечего
//если персонаж погиб, прекращаем отрисовку интерфейса
//если первая часть игры, интерфейс не отображается
//if ((player_obj == -1) or (obj_ctrl_gm_playerStatus.hp <= 0) or (obj_ctrl_gm.ctrl_gm_stage == 1)) { //!D
if ((player_obj == -1) or (obj_ctrl_gm_playerStatus.hp <= 0) or (obj_ctrl_gm.ctrl_gm_stage == 10)) { //D
	visible = 0;
} else {
	visible = 1;
}