	...
	for (var i = 0; i < ctrl_pixelRm_row; i++) {
		for (var j = 0; j < ctrl_pixelRm_col; j++) {
			...
			if (ctrl_pixelRm_arr_cl[i, j] != -1) {
				// меняю цвет 
				// ctrl_pixelRm_arr_cl[i, j] = (ctrl_pixelRm_arr_cl[i, j] + 80 / fps) mod 255;
...
draw_txt_xscale -= 0.5 / fps * ctrl_pixelRm_multiplier;
if (draw_txt_xscale == -3 or draw_txt_xscale == 3) {
	ctrl_pixelRm_multiplier *= -1;
}
*/
// меняю цвет
draw_txt_cl = draw_txt_cl + 31 / fps * ctrl_pixelRm_multiplier_cl;
