/// @description выполнение действия action

// получаю название спрайта без индекса
var spr_name = sprite_get_name(sprite_index);
spr_name = string_copy(spr_name, 0, string_length(spr_name) - 1);

switch (action) {
	// когда игрок нажимает на журнал и тот открывается
	case "openJournal":
		// рисуем фон журнала
		visible = true;
		// создаю кнопки-закладки
		// все слагаемые и вычитаемые - константы для красивого положения кнопок
		var t1 = room_width * 0.24 - sprite_get_bbox_left(spr_journal1) + sprite_get_bbox_right(spr_journal1);
		var t2 = room_height * 0.36;
		var t3 = sprite_get_height(spr_btn_journal_history1) + 10;
		// создаю закладки
		var obj = scr_btn_create(room_width * 0.2, t2, spr_btn_journal_history1, depth + 1, 0, "journalHistory", "func", [20, "snd_none", "snd_menu_journalPage"]);
		ds_map_add(ctrl_jrn_map_btn, "history", [obj, true, 1]);
		var obj = scr_btn_create(t1, t2 + t3, spr_btn_journal_quest1, depth + 1, 0, "journalQuest", "func", [20, "snd_none", "snd_menu_journalPage"]);
		ds_map_add(ctrl_jrn_map_btn, "quest", [obj, false, 1]);
		var obj = scr_btn_create(t1, t2 + t3 * 2, spr_btn_journal_monsterInfo1, depth + 1, 0, "journalMonsterInfo", "func", [20, "snd_none", "snd_menu_journalPage"]);
		ds_map_add(ctrl_jrn_map_btn, "monsterInfo", [obj, false, 1]);
		var obj = scr_btn_create(t1, t2 + t3 * 3, spr_btn_journal_weaponInfo1, depth + 1, 0, "journalWeaponInfo", "func", [20, "snd_none", "snd_menu_journalPage"]);
		ds_map_add(ctrl_jrn_map_btn, "weaponInfo", [obj, false, 1]);
		var obj = scr_btn_create(t1, t2 + t3 * 4, spr_btn_journal_achiv1, depth + 1, 0, "journalAchiv", "func", [20, "snd_none", "snd_menu_journalPage"]);
		ds_map_add(ctrl_jrn_map_btn, "achiv", [obj, false, 1]);
		// стрелочки страниц
		var obj = scr_btn_create(room_width * 0.65, room_height * 0.8, spr_btn_journal_next1, depth - 1, 0, "journalNext", "func", [20, "snd_none", "snd_menu_journalPage"]);
		// сохраняю стрелки, чтобы удалять их во время перелистывания 
		ctrl_jrn_btn_arrow_arr[0] = obj;
		var obj = scr_btn_create(room_width * 0.33, room_height * 0.8, spr_btn_journal_pre1, depth - 1, 0, "journalPre", "func", [20, "snd_none", "snd_menu_journalPage"]);
		ctrl_jrn_btn_arrow_arr[1] = obj;
		// подгружаем текст из файла
		event_user(14);
	break;  
	// перелистывание страницы раздела
	case "journalNext":
		if (array_length(ctrl_jrn_btn_arrow_arr) != 0) {
			// получаю текущую страницу и максимальное количество страниц
			var t = ctrl_jrn_map_btn[?ctrl_jrn_btn_active];
			var t2 = ctrl_jrn_map_txt[?ctrl_jrn_btn_active];
			// если страница не последняя
			if (t[2] + 2 <= ceil(array_length(t2) / ctrl_jrn_txt_str)) {
				t[2] += 2;	// у нас ведь разворот и перелистывается сразу две страницы
				// спрайт перелистывания страницы
				sprite_index = spr_journal_page;
				scr_snd_requestPlaySnd(20, "snd_menu_journalPage", "snd_menu_journalPage", true);	// звук страницы
				image_speed = 1;
				// стрелочки на страницах не должны существовать во время перелистывания
				instance_destroy(ctrl_jrn_btn_arrow_arr[0]);
				instance_destroy(ctrl_jrn_btn_arrow_arr[1]);
			}
			// переносим измененные данные (напрямую не позволяет движок)
			array_copy(ctrl_jrn_map_btn[?ctrl_jrn_btn_active], 0, t, 0, array_length(t));
		}
	break;
	// перелистывание страницы раздела
	case "journalPre":
		if (array_length(ctrl_jrn_btn_arrow_arr) != 0) {
			// получаю текущую страницу и максимальное количество страниц
			var t = ctrl_jrn_map_btn[?ctrl_jrn_btn_active];
			var t2 = ctrl_jrn_map_txt[?ctrl_jrn_btn_active];
			// если страница не последняя
			if (t[2] - 2 >= 1) {
				t[2] -= 2;
				// спрайт перелистывания страницы
				sprite_index = spr_journal_page;
				scr_snd_requestPlaySnd(20, "snd_menu_journalPage", "snd_menu_journalPage", true);	// звук страницы
				image_index = image_number - 0.1;
				image_speed = -1;
				// стрелочки на страницах не должны существовать во время перелистывания
				instance_destroy(ctrl_jrn_btn_arrow_arr[0]);
				instance_destroy(ctrl_jrn_btn_arrow_arr[1]);
			}
			// переносим измененные данные (напрямую не позволяет движок)
			array_copy(ctrl_jrn_map_btn[?ctrl_jrn_btn_active], 0, t, 0, array_length(t));
		}
	break;
	// перемещение по разделам
	case "journalHistory":
		// проверяем, что мы сейчас не находимся в процессе перелистывания закладки или страниц вообще
		if (spr_name != "spr_journal_page") {
			// переходим в раздел истории
			scr_change_chapter("history", 1);
		}
	break;
	case "journalQuest":
		// проверяем, что мы сейчас не находимся в процессе перелистывания закладки или страниц вообще
		if (spr_name != "spr_journal_page") {
			scr_change_chapter("quest", 2);
		}
	break;
	case "journalMonsterInfo":
		// проверяем, что мы сейчас не находимся в процессе перелистывания закладки или страниц вообще
		if (spr_name != "spr_journal_page") {
			scr_change_chapter("monsterInfo", 3);
		}
	break;
	case "journalWeaponInfo":
		// проверяем, что мы сейчас не находимся в процессе перелистывания закладки или страниц вообще
		if (spr_name != "spr_journal_page") {
			scr_change_chapter("weaponInfo", 4);
		}
	break;
	case "journalAchiv":
		// проверяем, что мы сейчас не находимся в процессе перелистывания закладки или страниц вообще
		if (spr_name != "spr_journal_page") {
			scr_change_chapter("achiv", 5);
		}
	break;
}